...
pub fn derive_ser_json_impl(input: TokenStream) -> TokenStream {
    ...
    if parser.eat_ident("struct"){
        if let Some(name) = parser.eat_any_ident(){
            ...
            tb.add("{ fn ser_json ( & self , d : usize , s : & mut microserde :: SerJsonState ) {");
...
pub fn derive_de_json_impl(input: TokenStream) -> TokenStream {
    ...
    if parser.eat_ident("struct"){
        if let Some(name) = parser.eat_any_ident(){
            ...
            tb.add("{ fn de_json ( s : &  mut microserde :: DeJsonState , i : & mut std :: str :: Chars )");
            tb.add("-> std :: result :: Result < Self , microserde :: DeJsonErr > { ");
