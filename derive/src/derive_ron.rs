...
pub fn derive_ser_ron_impl(input: TokenStream) -> TokenStream {
    ...
    if parser.eat_ident("struct"){
        if let Some(name) = parser.eat_any_ident(){
            ...
            tb.add("{ fn ser_ron ( & self , d : usize , s : & mut microserde :: SerRonState ) {");
...
pub fn derive_de_ron_impl(input: TokenStream) -> TokenStream {
    ...
    if parser.eat_ident("struct"){
        if let Some(name) = parser.eat_any_ident(){
            ...
            tb.add("{ fn de_ron ( s : &  mut microserde :: DeRonState , i : & mut std :: str :: Chars )");
            tb.add("-> std :: result :: Result < Self , microserde :: DeRonErr > { ");
